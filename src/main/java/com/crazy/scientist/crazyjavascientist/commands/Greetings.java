...
public class Greetings extends ListenerAdapter {
    ...
    private final List<String> theJavaWayGreetings = new ArrayList<>();
    ...
    public void onGuildMemberJoin(@NotNull GuildMemberJoinEvent event) {
        ...
        if (event.getJDA().getGuildById(952394376640888853L).getName().equalsIgnoreCase(event.getGuild().getName())) {
            event.getGuild().getDefaultChannel().asTextChannel().sendMessageFormat("Welcome to %s!  %s  will you be able to climb your way up the chain", event.getGuild().getName(), event.getUser().getName()).queue();
        }
        if (event.getJDA().getGuildById(1008992976090963988L).getName().equalsIgnoreCase(event.getGuild().getName())) {
            event.getGuild().getDefaultChannel().asTextChannel().sendMessage(theJavaWayGreetings.get((int) (Math.random() * theJavaWayGreetings.size()))).queue();
        }
    }
}
